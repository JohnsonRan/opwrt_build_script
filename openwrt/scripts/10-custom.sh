...
if curl -s "$mirror/openwrt/24-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-momo=y"; then
    ...
    echo "src/gz nikki https://momomomo.pages.dev/openwrt-24.10/$arch/momo" >>files/etc/opkg/customfeeds.conf
...
# tailscale
if curl -s "$mirror/openwrt/24-config-common" | grep -q "^CONFIG_PACKAGE_tailscale=y"; then
...
if [[ "$arch" == "x86_64" ]]; then
    curl -skLo package/base-files/files/etc/banner https://github.com/JohnsonRan/opwrt_build_script/raw/master/openwrt/files/etc/banner-x86
else
