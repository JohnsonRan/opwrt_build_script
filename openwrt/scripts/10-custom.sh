...
if curl -s "$mirror/openwrt/24-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-momo=y"; then
    ...
    rm -rf gh-pages.zip
    curl -skL https://raw.githubusercontent.com/JohnsonRan/opwrt_build_script/master/openwrt/patch/sing-box/reF1nd.patch | git -C package/new/openwrt-momo/momo apply
fi
...
# luci-app-podman
git clone https://github.com/Zerogiven-OpenWRT-Packages/luci-app-podman package/new/luci-app-podman --depth=1
...
rm -rf package/new/packages/{openwrt-fchomo,openwrt-Natter}
rm -rf package/new/luci/{luci-app-netspeedtest,luci-app-natter}
rm -rf package/fantastic_packages

# cool sysctl
curl -so files/etc/sysctl.d/99-custom.conf $mirror/openwrt/files/etc/sysctl.d/99-custom.conf
