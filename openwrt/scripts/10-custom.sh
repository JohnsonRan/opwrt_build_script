...
if curl -s "$mirror/openwrt/24-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-momo=y"; then
    git clone https://$github/nikkinikki-org/OpenWrt-momo package/new/luci-app-momo --depth=1
    ...
    curl -skLo files/etc/momo/run/ui/gh-pages.zip https://$github/Zephyruso/zashboard/archive/refs/heads/gh-pages.zip
    unzip gh-pages.zip -d files/etc/momo/run/ui/zashboard
    rm -rf gh-pages.zip
...
# luci-app-3cat
git clone https://$github/immortalwrt/luci immsource/immortalwrt_luci --depth=1
git clone https://$github/immortalwrt/packages immsource/immortalwrt_packages --depth=1
...
# FANTASTIC PACKAGES
git clone --depth=1 https://github.com/fantastic-packages/packages package/new/fantastic-packages
git clone --depth=1 https://github.com/muink/openwrt-fastfetch package/new/fastfetch
