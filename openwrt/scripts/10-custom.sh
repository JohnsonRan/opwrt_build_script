...
if curl -s "$mirror/openwrt/25-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-nikki=y"; then
    ...
    echo "https://nikkinikki.pages.dev/openwrt-25.12/x86_64/nikki/packages.adb" >>files/etc/apk/repositories.d/customfeeds.list
    ...
    sed -i "s/PKG_SOURCE_VERSION:=.*/PKG_SOURCE_VERSION:=Alpha/" package/new/openwrt-nikki/mihomo-alpha/Makefile
    sed -i "s/PKG_MIRROR_HASH:=.*/PKG_MIRROR_HASH:=skip/" package/new/openwrt-nikki/mihomo-alpha/Makefile
    sed -i "/PKG_BUILD_FLAGS/i PKG_BUILD_VERSION:=$nikki_version" package/new/openwrt-nikki/mihomo-alpha/Makefile
    sed -i 's|GO_PKG_LDFLAGS_X:=$(GO_PKG)/constant.Version=$(PKG_SOURCE_VERSION)|GO_PKG_LDFLAGS_X:=$(GO_PKG)/constant.Version=$(PKG_BUILD_VERSION)|g' package/new/openwrt-nikki/mihomo-alpha/Makefile
fi
...
if curl -s "$mirror/openwrt/25-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-momo=y"; then
    ...
    echo "https://momomomo.pages.dev/openwrt-25.12/x86_64/momo/packages.adb" >>files/etc/apk/repositories.d/customfeeds.list
...
git clone --depth=1 https://github.com/Tokisaki-Galaxy/luci-app-tailscale-community package/new/luci-app-tailscale-community
...
# FANTASTIC PACKAGES
git clone --depth 1 --branch master --single-branch --no-tags --recurse-submodules https://github.com/JohnsonRan/packages package/fantastic_packages
